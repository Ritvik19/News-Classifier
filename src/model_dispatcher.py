...
def save_results_classification(classification_report, confusion_matrix, performance, name, dataset_id, problem=None):
    ...
    with open(os.path.join(output_directory, 'Classification-Report.txt'), 'w') as f:
        ...
        f.write(f'<tr><th>Accuracy</th><td>{round(np.mean(performance["accuracy"], axis=0)[50], 4)}</td></tr>\n')
        f.write(f'<tr><th>Max Accuracy</th><td>{round(np.max(np.mean(performance["accuracy"], axis=0)), 4)} ({np.argmax(np.mean(performance["accuracy"], axis=0))})</td></tr>\n')
